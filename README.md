...
운영 보드에 쌓인 그날의 매출·방문·문의·사물함·OT를 매일 **22:00(한국시간)** 에
텔레그램으로 자동 전송합니다. GitHub Actions가 실행하므로 **추가 비용도, 카드 등록도 없습니다.**

동작: GitHub이 매일 밤 10시에 `scripts/daily-report.mjs` 실행 → Firebase에 익명으로
접속해 그날 데이터를 읽음 → 집계해서 텔레그램으로 발송. (Firebase 설정값은
`public/firebase-config.js` 에서 자동으로 읽으므로 따로 넣을 필요 없습니다.)
...
**3. GitHub에 비밀값 2개 저장**
1. https://github.com/jiyoon7653/-/settings/secrets/actions 접속
...
- 저장소에 **60일간 아무 변화가 없으면 예약 실행이 자동 중지**됩니다.
  ...
  다시 활성화하면 됩니다.
- 발송 시각을 바꾸려면 `.github/workflows/daily-report.yml` 의 `cron: '0 13 * * *'`
