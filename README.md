...
- **매니저(고범수) 당월 총매출 칸**: 표 아래 "당월 총매출" 에 그 달 총매출을 넣으면,
  그 금액이 속한 **매출 구간의 요율**로 인센티브가 자동 계산됩니다(예: 3,000만원 이상 → 10%).
  칸을 비워두면 위 세션 표에서 개인매출을 뺀 합계로 구간을 판정합니다.
  구간 경계값·요율은 관리자 화면의 "매출 구간별 인센티브 요율" 에서 조정합니다.
