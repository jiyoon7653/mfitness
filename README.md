...
- **이번 달 매출 달성률**과 **이번 주 매출 달성률**을 개요 탭·매출 탭 양쪽에서 각각 확인할
  수 있습니다. "목표 대비 달성" 표의 이번 주/이번 달 토글도 이제 각자의 실제 목표를 씁니다.
...
**운영 보드 사용법 — OT 관리 탭 (새로 추가)**
- **트레이너별 월 목표**를 "트레이너·목표 수정"에서 관리합니다. 트레이너 추가/삭제도
  여기서 합니다.
- **OT 진행 등록** 폼에 진행할 때마다 한 건씩 입력합니다: 담당 트레이너, 고객명,
  연락처, 진행상황(예약/진행중/완료/노쇼), 전환여부(미정/전환완료/미전환).
- 위쪽 KPI에 **이번 달 목표·진행 건수·달성률·전환율**이 자동으로 집계되고,
  그 아래 표에서 **트레이너별로도** 같은 지표를 따로 볼 수 있습니다.
...
- 개요 탭의 "이번 달 OT 누적" 타일도 이제 이 데이터를 그대로 씁니다 — 예전처럼
  손으로 적어 넣는 숫자가 아니라 실제 등록된 인원 수입니다.
...
🏋️ 이번 달 OT 달성률  20% (3/15건 · 전환 1건)
```

OT 달성률 줄은 OT 관리 탭에서 트레이너 목표를 하나라도 설정해야 나타납니다.
