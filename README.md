...
| `lockers` | 사물함 (건별 문서) |
| `otSessions` | OT 진행 인원 (건별 문서: 트레이너, 고객명, 연락처, 진행상황, 전환여부) |
| `settings/salesCategories` | 매출 카테고리와 월/주 목표 |
| `settings/otTrainers` | 트레이너별 OT 월 목표 |
| `settings/monthlyFocus` | 이달의 집중 항목 |

여러 명이 동시에 봐도 실시간으로 같이 갱신됩니다 (`onSnapshot`).

**운영 보드 사용법 — 매출·업무일지 탭**
- **월 목표 · 주 목표를 카테고리별로 따로 설정**합니다. "카테고리·목표 수정"을 누르면
  카테고리마다 월 목표, 주 목표 두 칸이 나옵니다. 주 목표는 월 목표에서 자동으로
  나누는 게 아니라 직접 입력한 값을 그대로 씁니다 (주마다 목표가 다를 수 있으니까요).
...
**운영 보드 사용법 — OT 관리 탭**
- **트레이너별 월 목표 · 주 목표**를 "트레이너·목표 수정"에서 관리합니다 (매출
  카테고리와 같은 방식). 트레이너 추가/삭제도 여기서 합니다.
